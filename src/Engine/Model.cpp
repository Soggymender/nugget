...
void Model::loadModel(string const& path)
{
    ...
    processNode(scene->mRootNode, scene, workingDir);
}

// processes a node in a recursive fashion. Processes each individual mesh located at the node and repeats this process on its children nodes (if any).
void Model::processNode(aiNode* node, const aiScene* scene, const string& workingDir)
{
    ...
    for (unsigned int i = 0; i < node->mNumMeshes; i++)
    {
        ...
        meshes.push_back(processMesh(mesh, scene, workingDir));
    }
    // after we've processed all of the meshes (if any) we then recursively process each of the children nodes
    for (unsigned int i = 0; i < node->mNumChildren; i++)
    {
        processNode(node->mChildren[i], scene, workingDir);
    }

}

Mesh Model::processMesh(aiMesh* mesh, const aiScene* scene, const string& workingDir)
{
    // data to fill
...
vector<Texture*> Model::processMaterialTextures(aiMaterial* mat, aiTextureType type, string typeName, const string& workingDir)
{
    vector<Texture*> textures;
